...
int ITEMisfrontale(Item_t p){
    if (p.ingresso == frontale)     return 1;
    else                            return 0;
}
...
int ITEMsequenza(Item_t p1, Item_t p2){  //usato in debug
    if (p1.tipologia > transazione && p2.tipologia >transazione)    return 1;
    else                                                            return 0;
}
